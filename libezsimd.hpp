...
namespace ezsimd {
    ...
    #pragma region // add
        #pragma region // int8_t
            ...
            template <size_t S>
            inline void add(const std::array<int8_t, S>& a, const std::array<int8_t, S>& b, std::array<int8_t, S>& c) {
        ...
        #pragma region // int16_t
            ...
            template <size_t S>
            inline void add(const std::array<int16_t, S>& a, const std::array<int16_t, S>& b, std::array<int16_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int32_t
            ...
            template <size_t S>
            inline void add(const std::array<int32_t, S>& a, const std::array<int32_t, S>& b, std::array<int32_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int64_t
            ...
            template <size_t S>
            inline void add(const std::array<int64_t, S>& a, const std::array<int64_t, S>& b, std::array<int64_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __int128_t
            ...
            template <size_t S>
            inline void add(const std::array<__int128_t, S>& a, const std::array<__int128_t, S>& b, std::array<__int128_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint8_t
            ...
            template <size_t S>
            inline void add(const std::array<uint8_t, S>& a, const std::array<uint8_t, S>& b, std::array<uint8_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint16_t
            ...
            template <size_t S>
            inline void add(const std::array<uint16_t, S>& a, const std::array<uint16_t, S>& b, std::array<uint16_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint32_t
            ...
            template <size_t S>
            inline void add(const std::array<uint32_t, S>& a, const std::array<uint32_t, S>& b, std::array<uint32_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint64_t
            ...
            template <size_t S>
            inline void add(const std::array<uint64_t, S>& a, const std::array<uint64_t, S>& b, std::array<uint64_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __uint128_t
            ...
            template <size_t S>
            inline void add(const std::array<__uint128_t, S>& a, const std::array<__uint128_t, S>& b, std::array<__uint128_t, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // float
            ...
            template <size_t S>
            inline void add(const std::array<float, S>& a, const std::array<float, S>& b, std::array<float, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // double
            ...
            template <size_t S>
            inline void add(const std::array<double, S>& a, const std::array<double, S>& b, std::array<double, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // long double
            ...
            template <size_t S>
            inline void add(const std::array<long double, S>& a, const std::array<long double, S>& b, std::array<long double, S>& c) {
                addBackend(a.data(), b.data(), c.data(), a.size());
            }
    ...
    #pragma region // sub
        #pragma region // int8_t
            ...
            template <size_t S>
            inline void sub(const std::array<int8_t, S>& a, const std::array<int8_t, S>& b, std::array<int8_t, S>& c) {
        ...
        #pragma region // int16_t
            ...
            template <size_t S>
            inline void sub(const std::array<int16_t, S>& a, const std::array<int16_t, S>& b, std::array<int16_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int32_t
            ...
            template <size_t S>
            inline void sub(const std::array<int32_t, S>& a, const std::array<int32_t, S>& b, std::array<int32_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int64_t
            ...
            template <size_t S>
            inline void sub(const std::array<int64_t, S>& a, const std::array<int64_t, S>& b, std::array<int64_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __int128_t
            ...
            template <size_t S>
            inline void sub(const std::array<__int128_t, S>& a, const std::array<__int128_t, S>& b, std::array<__int128_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint8_t
            ...
            template <size_t S>
            inline void sub(const std::array<uint8_t, S>& a, const std::array<uint8_t, S>& b, std::array<uint8_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint16_t
            ...
            template <size_t S>
            inline void sub(const std::array<uint16_t, S>& a, const std::array<uint16_t, S>& b, std::array<uint16_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint32_t
            ...
            template <size_t S>
            inline void sub(const std::array<uint32_t, S>& a, const std::array<uint32_t, S>& b, std::array<uint32_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint64_t
            ...
            template <size_t S>
            inline void sub(const std::array<uint64_t, S>& a, const std::array<uint64_t, S>& b, std::array<uint64_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __uint128_t
            ...
            template <size_t S>
            inline void sub(const std::array<__uint128_t, S>& a, const std::array<__uint128_t, S>& b, std::array<__uint128_t, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // float
            ...
            template <size_t S>
            inline void sub(const std::array<float, S>& a, const std::array<float, S>& b, std::array<float, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // double
            ...
            template <size_t S>
            inline void sub(const std::array<double, S>& a, const std::array<double, S>& b, std::array<double, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // long double
            ...
            template <size_t S>
            inline void sub(const std::array<long double, S>& a, const std::array<long double, S>& b, std::array<long double, S>& c) {
                subBackend(a.data(), b.data(), c.data(), a.size());
            }
    ...
    #pragma region // mul
        #pragma region // int8_t
            ...
            template <size_t S>
            inline void mul(const std::array<int8_t, S>& a, const std::array<int8_t, S>& b, std::array<int8_t, S>& c) {
        ...
        #pragma region // int16_t
            ...
            template <size_t S>
            inline void mul(const std::array<int16_t, S>& a, const std::array<int16_t, S>& b, std::array<int16_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int32_t
            ...
            template <size_t S>
            inline void mul(const std::array<int32_t, S>& a, const std::array<int32_t, S>& b, std::array<int32_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int64_t
            ...
            template <size_t S>
            inline void mul(const std::array<int64_t, S>& a, const std::array<int64_t, S>& b, std::array<int64_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __int128_t
            ...
            template <size_t S>
            inline void mul(const std::array<__int128_t, S>& a, const std::array<__int128_t, S>& b, std::array<__int128_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint8_t
            ...
            template <size_t S>
            inline void mul(const std::array<uint8_t, S>& a, const std::array<uint8_t, S>& b, std::array<uint8_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint16_t
            ...
            template <size_t S>
            inline void mul(const std::array<uint16_t, S>& a, const std::array<uint16_t, S>& b, std::array<uint16_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint32_t
            ...
            template <size_t S>
            inline void mul(const std::array<uint32_t, S>& a, const std::array<uint32_t, S>& b, std::array<uint32_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint64_t
            ...
            template <size_t S>
            inline void mul(const std::array<uint64_t, S>& a, const std::array<uint64_t, S>& b, std::array<uint64_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __uint128_t
            ...
            template <size_t S>
            inline void mul(const std::array<__uint128_t, S>& a, const std::array<__uint128_t, S>& b, std::array<__uint128_t, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // float
            ...
            template <size_t S>
            inline void mul(const std::array<float, S>& a, const std::array<float, S>& b, std::array<float, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // double
            ...
            template <size_t S>
            inline void mul(const std::array<double, S>& a, const std::array<double, S>& b, std::array<double, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // long double
            ...
            template <size_t S>
            inline void mul(const std::array<long double, S>& a, const std::array<long double, S>& b, std::array<long double, S>& c) {
                mulBackend(a.data(), b.data(), c.data(), a.size());
            }
    ...
    #pragma region // div
        #pragma region // int8_t
            ...
            template <size_t S>
            inline void div(const std::array<int8_t, S>& a, const std::array<int8_t, S>& b, std::array<int8_t, S>& c) {
        ...
        #pragma region // int16_t
            ...
            template <size_t S>
            inline void div(const std::array<int16_t, S>& a, const std::array<int16_t, S>& b, std::array<int16_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int32_t
            ...
            template <size_t S>
            inline void div(const std::array<int32_t, S>& a, const std::array<int32_t, S>& b, std::array<int32_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // int64_t
            ...
            template <size_t S>
            inline void div(const std::array<int64_t, S>& a, const std::array<int64_t, S>& b, std::array<int64_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __int128_t
            ...
            template <size_t S>
            inline void div(const std::array<__int128_t, S>& a, const std::array<__int128_t, S>& b, std::array<__int128_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint8_t
            ...
            template <size_t S>
            inline void div(const std::array<uint8_t, S>& a, const std::array<uint8_t, S>& b, std::array<uint8_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint16_t
            ...
            template <size_t S>
            inline void div(const std::array<uint16_t, S>& a, const std::array<uint16_t, S>& b, std::array<uint16_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint32_t
            ...
            template <size_t S>
            inline void div(const std::array<uint32_t, S>& a, const std::array<uint32_t, S>& b, std::array<uint32_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // uint64_t
            ...
            template <size_t S>
            inline void div(const std::array<uint64_t, S>& a, const std::array<uint64_t, S>& b, std::array<uint64_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // __uint128_t
            ...
            template <size_t S>
            inline void div(const std::array<__uint128_t, S>& a, const std::array<__uint128_t, S>& b, std::array<__uint128_t, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // float
            ...
            template <size_t S>
            inline void div(const std::array<float, S>& a, const std::array<float, S>& b, std::array<float, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // double
            ...
            template <size_t S>
            inline void div(const std::array<double, S>& a, const std::array<double, S>& b, std::array<double, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
        ...
        #pragma region // long double
            ...
            template <size_t S>
            inline void div(const std::array<long double, S>& a, const std::array<long double, S>& b, std::array<long double, S>& c) {
                divBackend(a.data(), b.data(), c.data(), a.size());
            }
